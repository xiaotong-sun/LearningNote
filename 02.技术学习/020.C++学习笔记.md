...
问题描述：羊类、驼类继承了动物类，而羊驼又继承了羊类和驼类，这就导致动物类中的一些属性被羊驼类继承了两次，造成数据浪费，也会产生二义性。

问题解决：虚继承
```c++
class Sheep : virtual public Animal {}; // 继承前加virtual关键词之后，变为虚继承
class Tuo : virtual public Animal {};	// 此时公共的父类Animal被称为虚基类
class SheepTuo : public Sheep, public Tuo {};
```

虚继承的工作原理：

Sheep类和Tuo类存储了一个虚基类指针，该指针指向一个虚基类表，虚基类表中存着一些变量的偏移量，通过该偏移量可以正确地访问到具体的数据。
...
- 静态多态的函数地址早绑定：在编译阶段确定函数的地址。
- 动态多态的函数地址晚绑定：在运行阶段确定函数的地址。
...
class Animal {
public:
    virtual void speak() { // 虚函数，子类需重写虚函数
        cout << "Animal" << endl;
    }
};
...
借助虚函数表来进行控制，每个类会存储一个虚函数指针，指向一个虚函数表，当子类重写父类的虚函数时，子类对应的虚函数表内部会子类的虚函数地址。

<img src="./../99.Figure/02-020/image-20240221144622377.png" alt="image-20240221144622377" />
...
纯虚函数的语法：`virtual void test01(int a, int b) = 0;`
...
多态使用时，如果子类中有属性开辟到堆区，那么父类指针在释放的时候无法调用子类的析构代码，导致堆区的数据无法释放，内存泄漏。
...
| ios::ate    | 初始位置：文件末尾                     |
| ios::app    | 以追加方式写文件                       |
| ios::trunc  | 如果文件存在，则删除该文件之后重新创建 |
| ios::binary | 以二进制方式                           |

**注意：**文件打开方式可以配合使用，利用`|`符号进行连接
...
4. 读数据
    ...
    - ```c++
        char buf[1024] = {0};
        while (ifs.getline(buf, sizeof(buf))) { // 以换行符为分割依据
        	cout << buf << endl;
        }
        ```
    ...
    - ```c++
        string buf;
        while (getline(ifs, buf)) { // 以换行符为分割依据
            cout << buf << endl;
        }
        ```
...
// template -- 声明创建模板
// typename -- 标明其后面的符号是一种数据类型，可以用class代替
...
- 函数模板使用关键字：template
- 函数模板的使用方式有两种：1.自动类型推导；2.显式指定类型
- 函数模板能够提高代码的复用性，将类型参数化、



#### 1.2.2 函数模板的注意事项

- 自动类型推导，必须推导出一致的数据类型T才能够使用。
...
3. 函数模板也可以发生重载。
4. 如果函数模板相较于普通函数能发生更好的匹配，则优先调用函数模板。
...
显示具体化（也称为显式特化）的概念，这是模板编程中的一个重要特性，允许开发者为特定类型提供特定的实现。你的问题是，既然可以为特定类型提供特定的函数实现，为什么还要使用模板而不是直接写成一个针对该类型的函数呢？

这个问题的答案涉及到模板在代码复用、类型安全、维护性以及性能优化中的优势：

1. **代码复用和泛化**：最主要的原因是，函数模板允许编写通用的代码，这些代码可以工作在多种数据类型上。这意味着对于大多数使用场景，你可以写一次代码，然后用于多种类型，而不是为每种类型重复编写类似功能的代码。即使对于特定类型需要特殊实现的情况，大部分的函数逻辑仍然可以通过模板复用。
2. **类型安全**：使用函数模板可以在编译时进行类型检查，这可以保证类型的正确使用和提供更高的类型安全性。对于显式具体化的函数，这仍然保持为真，因为它允许针对特定类型进行优化或特殊处理，同时维持模板提供的类型安全性。
3. **维护性**：即使对于某个特定类型我们需要一个特殊的实现，使用显示具体化的函数模板而不是一个普通函数，依然可以保持代码的整洁和一致性。在大型项目中，这有助于维护代码的模块化和可读性。如果将来需要修改逻辑，可能只需要修改一处代码而不是多处。
4. **潜在的性能优化**：编译器对模板的实例化进行优化的能力通常超过手工编写的代码，因为编译器在具体化模板时有更多关于上下文的信息，可以进行更激进的优化。特别是在模板函数体很小时，内联这样的函数模板可能比手工编写的函数更有效。
5. **显式具体化提供特殊案例的灵活性**：有时候，模板函数对于某些特定的类型可能需要特殊的实现逻辑以优化性能或处理特殊行为。在这种情况下，显示具体化允许保留模板的全部好处，同时针对特定情况提供高效或特殊的实现。

总而言之，即使在存在显式具体化（特殊实现）的情况下，函数模板提供的泛化、类型安全、重用性和维护性等优点，都是单纯使用普通函数所无法比拟的。这就是为什么在许多情况下即使可以具体化某个函数模板，使用模板本身仍然是首选方式的原因。
...
- 建立一个通用的类，类中成员的数据类型可以不具体指定，用一个虚拟的类型来代表。
...
public:
    ...
    void showMes() {
        cout << "CAT" << endl;
        cout << typeid(this->name).name() << " + " << typeid(this->age).name();
    }
};

template <typename T1, typename T2>
void show(Animal<T1, T2>& d) {
    d.showMes();
}
...
// 成员函数的类外实现
template <typename T1, typename T2>
void Animal<T1, T2>::showMes() {
    cout << typeid(T1).name() << " + " << typeid(T2).name() << endl;
}

int main() {
    Animal<string, int> animal("aaa", 32);
    animal.showMes();
}
```
...
类模板中的成员函数创建时机是在调用阶段，因此编译时编译器只能看到模板类的声明，模板类的具体函数的实现代码是看不到的，因此在链接的时候链接不到。

具体细节：

对于正常的类（非模板类），通常可以将类的声明和定义分开放在不同的头文件和源文件中而不会导致问题，因为编译器会直接将类的定义编译成目标代码，并在链接阶段将这些目标代码链接在一起。

当你包含一个正常的类的头文件时，编译器会将类的定义直接插入到包含了头文件的源文件中。因为类的定义是完整的，包括类的成员函数的实现，所以编译器能够在编译时生成对应的目标代码。

而对于模板类，则有所不同：

1. **模板是泛化的描述**：模板类并不是真正的代码，它只是一种泛化的描述。当你实例化一个模板类时，编译器才会根据模板定义生成具体的代码。而编译器在编译源文件时，对于模板类的成员函数的实现并不会被实例化生成具体的代码。因此，如果你在一个源文件中包含了模板类的头文件，在编译该源文件时，编译器只是将模板成员函数的声明和定义查看为模板，并不会生成对应的具体实现代码。

    如果在另一个源文件中实例化了这个模板类，编译器会寻找对应的模板定义，但由于模板实例化时需要模板类的完整定义，而在之前的源文件中并没有生成对应的实现代码，所以编译器会在实例化阶段找不到定义而报错。

2. **模板在编译期表现出不同的特性**：模板类的成员函数的实现需要随着模板的具体化而生成，这种延迟生成的特性使得直接包含头文件可能会遇到编译问题。

==解决：==

- 方式1：直接包含.cpp源文件
- 方式2：将声明和实现写在同一个文件中，并更改后缀名为.hpp,hpp是约定的名称，并不是强制的。

```c++
#include <iostream>
// #include "Person.h" //-> 报错
// 因为，包含.h的时候，类模板中的成员函数是不会被创建的，它是在调用的时候才会被创建
// 第一种解决方法：直接包含源文件
#include "Person.cpp" // -> 成功

// 第二种解决方式：将.h和.cpp中的内容写在一起，将后缀名改为.hpp
#include "Person.hpp" // -> 成功
using namespace std;
...
全局函数类外实现 - 需要提前让编译器知道全局函数的存在（这种方式比较麻烦，不建议使用）
...
class Person {
    // 1. 全局函数 类内实现
    // 为什么它写在类内，却是一个全局函数？
    // 这可能是friend关键字 对于 类模板的特殊处理，会将其处理成一个全局函数。
...
- 可以对内置数据类型和自定义数据类型的数据进行存储。（使用具体化的函数模板处理自定义数据类型）
- 将数组中的数据存储到堆区。（使用new来实现）
- 构造函数中可以传入数组的容量。
- 提供对应的拷贝构造函数以及operator=防止浅拷贝问题。（运算符重载）
- 提供尾插法和尾删法对数组中的数据进行增加和删除。
- 可以通过下标的方式访问数组中的元素。 (运算符重载)
- 可以获取数组中当前元素个数和数组的容量。
...
public:
    ...
    MyArray(const MyArray& myarrs) {
        this->capacity = myarrs.capacity;
        this->size = myarrs.capacity;
        this->arr = new T[this->capacity];

        for (int i = 0; i < size; i++) {
            this->arr[i] = myarrs.arr[i];
        }
    }
...
质变算法：运算过程会更改区间内的元素内容

非质变算法：运算过程不会更改区间内的元素内容



迭代器，提供一种方法，使之能够依序访问某个容器中的元素，而无需暴露容器的内部表示方式。常用的迭代器种类为双向迭代器和随机访问迭代器。
...
- 函数对象使用重载的()时，由于其行为类似函数调用，因此也叫仿函数



**本质：**

函数对象(仿函数)是一个类，不是一个函数。其关键在于重载`()`运算符。
...
void test01() {
    ...
    vector<int>::iterator itor = find_if(v.begin(), v.end(), GreaterFive()); // 匿名函数调用
...
void test01() {
    ...
    sort(v.begin(), v.end(), greater<int>()); // 关系仿函数的使用
